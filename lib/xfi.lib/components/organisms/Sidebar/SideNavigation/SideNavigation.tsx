  ...
  return (
    <StyledSideNavigation className={className} ref={sidebarRef} $isOpen={isOpen}>
      ...
      <div className="itemsWrapper">
        {config.map(({ category, categoryTitle, items }, index) => (
          <Stack key={index} gap={isOpen ? 0 : '0.25rem'} width={'100%'} padding={'0 1.25rem'}>
        ...
        {isShowBottomSection && (
          <Stack alignItems={'flex-end'} width={'100%'} height={'100%'} justifyContent={'flex-end'} gap={'0.5rem'}>
            <Divider
              sx={{
            ...
            {isShowThemeToggle && (
              <Stack
                ...
                padding={isOpen ? '0.5rem 1.25rem 0' : '0.5rem 0.875rem 0'}
              >
                {isOpen && (
                  <Stack
                    direction={'row'}
            ...
            {bottomConfig?.map(({ items }, index) => (
              <Stack key={index} gap={isOpen ? 0 : '0.5rem'} width={'100%'} padding={'0 1.25rem'}>
                {renderItems(items)}
              </Stack>
            ))}
          </Stack>
